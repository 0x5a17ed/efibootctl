...
dist/$(BINARY_NAME): $(SRC) go.mod go.sum
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 GOAMD64=v2 go build -trimpath $(GO_LDFLAGS) -o $@ ./cmd/$(BINARY_NAME)

dist/$(BINARY_NAME).exe: $(SRC) go.mod go.sum
	CGO_ENABLED=0 GOOS=windows GOARCH=amd64 GOAMD64=v2 go build -trimpath $(GO_LDFLAGS) -o $@ ./cmd/$(BINARY_NAME)
